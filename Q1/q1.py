...
class node:
	...
	def __init__(self, children=None, col=-1, value=None, classes=None, ans=-1, isleaf=False, height=0,  numchild=0):
		#  tpassed1=0, correct1=0, tmajor1=0, tpassed2=0, correct2=0, tmajor2=0, tpassed3=0, correct3=0, tmajor3=0):
		...
		self.tpassed=[0,0,0,0]
		self.correct = [0, 0, 0, 0]
		self.tmajor = [0, 0, 0, 0]
		# VALIDATION
		# self.tpassed[1] = tpassed1 	# No. of observations reached this node while classifying
		# self.correct[1] = correct1 	# No. of observations correctly predicted by this SUBTREE
		# self.tmajor[1] = tmajor1 	# Total observations passed of majority class (ans)

		# # TEST
		# self.tpassed[2] = tpassed2 	# No. of observations reached this node while classifying
		# self.correct[2] = correct2 	# No. of observations correctly predicted by this SUBTREE
		# self.tmajor[2] = tmajor2 	# Total observations passed of majority class (ans)

		# # TRAIN
		# self.tpassed[3] = tpassed3 	# No. of observations reached this node while classifying
		# self.correct[3] = correct3 	# No. of observations correctly predicted by this SUBTREE
		# self.tmajor[3] = tmajor3 	#
...
def update_parents(nodex,p):
	if nodex.isleaf:
		return nodex.correct[p]
	else:
		temp=0
		for a in nodex.children:
			temp += update_parents(a,p)
		nodex.correct[p] = temp
		return nodex.correct[p]


def update_numchild(nodex):
	if nodex.isleaf:
		nodex.numchild = 0
	else:
		temp=0
...
def prune(nodex):
	...
	if nodex.isleaf:
		...
		# print((nodex,tempcorrect, cc, nodex.correct[1], nodex.tmajor[1], nodex.tpassed[1]))
		if tempcorrect > cc:
			bestnode = nodex
			# print("GOT ONE !!!!!!")
			cc = tempcorrect
...
initial = numnodes(decision_tree)
pruned=0
while True:
	better = False
	prune(decision_tree)
	if not better:
		...
		y1 = update_parents(decision_tree,1)
		y2 = update_parents(decision_tree,2)
		y3 = update_parents(decision_tree,3)
